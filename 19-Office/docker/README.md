# Base de démonstration `classeur` — MariaDB 12.3 LTS (Docker)

Conteneur MariaDB fournissant la base `classeur` (données échangées en .xlsx) du projet 19.
...
| Paramètre | Valeur |
|---|---|
| Hôte / Port | `127.0.0.1` / **`3324`** |
| Base | `classeur` |
| Compte applicatif | `classeur_app` / `classeur_pwd` |
| Administrateur | `root` / `root_demo_pwd` |

Valeurs lues par `src/NDX.Office.UI/App.config`. Mots de passe de **démonstration**.
...
| Table | Rôle |
|---|---|
| `enregistrement` | données : trois colonnes texte + date, à exporter/importer en .xlsx |

3 lignes sont amorcées.
