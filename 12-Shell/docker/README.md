# Base de démonstration `catalogue_liens` — MariaDB 12.3 LTS (Docker)

Conteneur MariaDB fournissant la base `catalogue_liens` (catalogue de raccourcis) du projet 12.
...
| Paramètre | Valeur |
|---|---|
| Hôte / Port | `127.0.0.1` / **`3317`** |
| Base | `catalogue_liens` |
| Compte applicatif | `catalogue_app` / `catalogue_pwd` |
| Administrateur | `root` / `root_demo_pwd` |

Valeurs lues par `src/NDX.Shell.UI/App.config`. Mots de passe de **démonstration**.
...
| Table | Rôle |
|---|---|
| `lien` | catalogue : catégorie (`application`/`web`), nom, cible (chemin ou URL), description |

5 liens sont amorcés (3 applications et 2 liens Web), exportables en raccourcis réels.
