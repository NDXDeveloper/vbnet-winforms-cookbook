# Base de démonstration `demarrage` — MariaDB 12.3 LTS (Docker)

Conteneur MariaDB fournissant la base `demarrage` (journal d'événements) du projet 25.
...
| Paramètre | Valeur |
|---|---|
| Hôte / Port | `127.0.0.1` / **`3330`** |
| Base | `demarrage` |
| Compte applicatif | `demarrage_app` / `demarrage_pwd` |
| Administrateur | `root` / `root_demo_pwd` |

Valeurs lues par `src/NDX.Demarrage.UI/App.config`. Mots de passe de **démonstration**.
...
| Table | Rôle |
|---|---|
| `evenement` | journal : type (étape / exception…), message, horodatage |

La table démarre **vide** : les événements sont produits par l'application
(séquence de démarrage, exceptions captées).
