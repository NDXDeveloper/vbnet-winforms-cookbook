# Base de démonstration `preferences` — MariaDB 12.3 LTS (Docker)

Conteneur MariaDB fournissant la base `preferences` (réglages clef/valeur) du projet 08.
...
| Paramètre | Valeur |
|---|---|
| Hôte / Port | `127.0.0.1` / **`3314`** |
| Base | `preferences` |
| Compte applicatif | `preferences_app` / `preferences_pwd` |
| Administrateur | `root` / `root_demo_pwd` |

Valeurs lues par `src/NDX.Controles.UI/App.config`. Mots de passe de **démonstration**.
...
| Table | Rôle |
|---|---|
| `preference` | réglages : clef (clef primaire), valeur, date de mise à jour |

3 préférences sont amorcées (état des interrupteurs et dernier zoom).
