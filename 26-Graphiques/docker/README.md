# Base de démonstration `mesures` — MariaDB 12.3 LTS (Docker)

Conteneur MariaDB fournissant la base `mesures` (série à tracer) du projet 26.
...
| Paramètre | Valeur |
|---|---|
| Hôte / Port | `127.0.0.1` / **`3331`** |
| Base | `mesures` |
| Compte applicatif | `mesures_app` / `mesures_pwd` |
| Administrateur | `root` / `root_demo_pwd` |

Valeurs lues par `src/NDX.Graphiques.UI/App.config`. Mots de passe de **démonstration**.
...
| Table | Rôle |
|---|---|
| `mesure` | série : libellé (axe), valeur, ordre d'affichage |

7 mesures sont amorcées (une série mensuelle, tracée en barres / courbe / points).
