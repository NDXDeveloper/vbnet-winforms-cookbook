# Base de démonstration `impressions` — MariaDB 12.3 LTS (Docker)

Conteneur MariaDB fournissant la base `impressions` (documents) du projet 16.
...
| Paramètre | Valeur |
|---|---|
| Hôte / Port | `127.0.0.1` / **`3321`** |
| Base | `impressions` |
| Compte applicatif | `impressions_app` / `impressions_pwd` |
| Administrateur | `root` / `root_demo_pwd` |

Valeurs lues par `src/NDX.Impression.UI/App.config`. Mots de passe de **démonstration**.
...
| Table | Rôle |
|---|---|
| `impression` | documents : titre, contenu, nombre de pages, date |

2 documents sont amorcés.
