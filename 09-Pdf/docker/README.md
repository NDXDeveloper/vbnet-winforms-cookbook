# Base de démonstration `bibliotheque` — MariaDB 12.3 LTS (Docker)

Conteneur MariaDB fournissant la base `bibliotheque` (PDF générés) du projet 09.
...
| Paramètre | Valeur |
|---|---|
| Hôte / Port | `127.0.0.1` / **`3315`** |
| Base | `bibliotheque` |
| Compte applicatif | `bibliotheque_app` / `bibliotheque_pwd` |
| Administrateur | `root` / `root_demo_pwd` |

Valeurs lues par `src/NDX.Pdf.UI/App.config`. Mots de passe de **démonstration**.
...
| Table | Rôle |
|---|---|
| `document_genere` | PDF archivés : titre, auteur, nb de pages, taille, contenu binaire, empreinte SHA-256 |

La table démarre **vide** : le binaire PDF valide est produit par l'application,
pas par SQL. Utilisez la page **« Composer un PDF »** puis **« Archiver en base »**.
