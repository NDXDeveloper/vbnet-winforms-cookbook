' ============================================================================
'  PageCatalogue.vb  -  Themes persistes en base.
...
''' <summary>Liste les thèmes en base et y enregistre les thèmes prédéfinis.</summary>
Public NotInheritable Class PageCatalogue
    ...
    Public Sub New()
        MyBase.New("Catalogue (base)", "Thèmes enregistrés (couleurs en hexadécimal). Vous pouvez (ré)enregistrer les thèmes prédéfinis.")
        Construire()
    End Sub

    Private Sub Construire()
        Dim haut As New FlowLayoutPanel() With {.Dock = DockStyle.Top, .Height = 44, .FlowDirection = FlowDirection.LeftToRight}
        haut.Controls.Add(Bouton("Enregistrer les thèmes prédéfinis", AddressOf SurEnregistrer))
    ...
    Private Sub SurEnregistrer(ByVal sender As Object, ByVal e As EventArgs)
        Try
            For Each theme As Theme In GestionnaireThemes.Predefinis()
                DepotTheme.Enregistrer(theme)
            Next
            _lblEtat.ForeColor = Color.Green
            _lblEtat.Text = "Thèmes prédéfinis enregistrés."
            SurLister(sender, e)
    ...
    Private Sub SurLister(ByVal sender As Object, ByVal e As EventArgs)
        Try
            _grille.DataSource = DepotTheme.ListerTable()
            If _lblEtat.Text = "" Then _lblEtat.Text = ""
        Catch ex As Exception
            _lblEtat.ForeColor = Color.Firebrick
            _lblEtat.Text = "Erreur : " & ex.Message
        End Try
