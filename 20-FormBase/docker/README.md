# Base de démonstration `themes` — MariaDB 12.3 LTS (Docker)

Conteneur MariaDB fournissant la base `themes` (thèmes visuels) du projet 20.
...
| Paramètre | Valeur |
|---|---|
| Hôte / Port | `127.0.0.1` / **`3325`** |
| Base | `themes` |
| Compte applicatif | `themes_app` / `themes_pwd` |
| Administrateur | `root` / `root_demo_pwd` |

Valeurs lues par `src/NDX.FormBase.UI/App.config`. Mots de passe de **démonstration**.
...
| Table | Rôle |
|---|---|
| `theme` | thèmes : nom (unique), couleurs fond/texte/accent en `#RRGGBB`, date |

2 thèmes sont amorcés (`Clair` et `Sombre`).
