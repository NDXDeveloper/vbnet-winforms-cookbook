# Base de démonstration `redaction` — MariaDB 12.3 LTS (Docker)

Conteneur MariaDB fournissant la base `redaction` (documents RTF) du projet 23.
...
| Paramètre | Valeur |
|---|---|
| Hôte / Port | `127.0.0.1` / **`3328`** |
| Base | `redaction` |
| Compte applicatif | `redaction_app` / `redaction_pwd` |
| Administrateur | `root` / `root_demo_pwd` |

Valeurs lues par `src/NDX.Editeur.UI/App.config`. Mots de passe de **démonstration**.
...
| Table | Rôle |
|---|---|
| `document` | documents : titre, contenu RTF (texte enrichi), date |

1 document est amorcé (« Bienvenue »).
