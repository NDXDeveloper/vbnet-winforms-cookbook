# Base de démonstration `boutique` — MariaDB 12.3 LTS (Docker)

Conteneur MariaDB fournissant la base `boutique` (catalogue de produits) du projet 05.
...
| Paramètre | Valeur |
|---|---|
| Hôte / Port | `127.0.0.1` / **`3311`** |
| Base | `boutique` |
| Compte applicatif | `boutique_app` / `boutique_pwd` |
| Administrateur | `root` / `root_demo_pwd` |

Valeurs lues par `src/NDX.Donnees.UI/App.config`. Mots de passe de **démonstration**.
...
| Table | Rôle |
|---|---|
| `produit` | catalogue : référence (unique), désignation, prix HT, stock, date de création |

15 produits de démonstration sont amorcés.
